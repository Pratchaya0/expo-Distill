...
async function getDB(): Promise<SQLiteDatabase> {
  if (!_db) _db = await SQLite.openDatabaseAsync('distill.db');
  return _db;
}
...
export async function initDB(): Promise<void> {
  ...
  await db.execAsync(`
    ...
    CREATE TABLE IF NOT EXISTS recordings (
      ...
      title TEXT NOT NULL, duration REAL NOT NULL DEFAULT 0,
      audioPath TEXT NOT NULL, transcript TEXT, summary TEXT,
      actionItems TEXT, mindMap TEXT, tags TEXT NOT NULL DEFAULT '[]',
      isFavorite INTEGER NOT NULL DEFAULT 0, language TEXT NOT NULL DEFAULT 'th',
      templateId TEXT, createdAt TEXT NOT NULL
    );
    CREATE TABLE IF NOT EXISTS notes (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      recordingId INTEGER NOT NULL, timestamp REAL NOT NULL DEFAULT 0,
      content TEXT NOT NULL DEFAULT '', imagePaths TEXT NOT NULL DEFAULT '[]'
    );
    CREATE TABLE IF NOT EXISTS templates (
      id TEXT PRIMARY KEY, name TEXT NOT NULL,
      description TEXT NOT NULL DEFAULT '', systemPrompt TEXT NOT NULL DEFAULT '',
  ...
  if (!(await FileSystem.getInfoAsync(imgDir)).exists)
    await FileSystem.makeDirectoryAsync(imgDir, { intermediates: true });
}

function rowToRecording(row: Record<string, unknown>): Recording & { id: number } {
  return {
    id: row.id as number, title: row.title as string,
    duration: row.duration as number, audioPath: row.audioPath as string,
...
export async function getRecording(id: number): Promise<(Recording & { id: number }) | null> {
  const db = await getDB();
  const row = await db.getFirstAsync<Record<string, unknown>>('SELECT * FROM recordings WHERE id = ?', id);
...
export async function updateRecording(id: number, patch: Partial<Omit<Recording, 'id'>>): Promise<void> {
  const sets: string[] = []; const vals: SQLiteBindValue[] = [];
...
export async function getNotes(recordingId: number): Promise<(Note & { id: number })[]> {
  ...
  return rows.map((r) => ({
    id: r.id as number, recordingId: r.recordingId as number,
    timestamp: r.timestamp as number, content: r.content as string,
...
export async function listTemplates(): Promise<PromptTemplate[]> {
  ...
  return rows.map((r) => ({
    ...
    description: r.description as string, systemPrompt: r.systemPrompt as string,
...
export async function saveTemplate(t: PromptTemplate): Promise<void> {
  const db = await getDB();
  await db.runAsync(
    'INSERT OR REPLACE INTO templates (id,name,description,systemPrompt,tags) VALUES (?,?,?,?,?)',
